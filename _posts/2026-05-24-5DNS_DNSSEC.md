...
| Mode | How it works |
|------|-------------|
| Recursive | Client asks DNS server → server <br> does all the work and <br>returns the final answer |
| Iterative | Server returns the <br>address of the *next* DNS server to ask → client <br>queries it directly |
...
**What is the ID in DNS?**

- Every DNS query includes a 16-bit transaction ID in the header.
When the resolver receives a response, it checks:
"Does the ID in this response match the ID I sent?"
If yes -- it accepts the answer.
- The 16-bit DNS transaction ID offers only 65,536 possible values.
An attacker can flood a resolver with forged responses covering all
possible IDs within the round-trip time of a real query, poisoning
the cache -- this is the Kaminsky DNS Cache Poisoning attack.
